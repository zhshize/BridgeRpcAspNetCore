...
namespace BridgeRpc.AspNetCore.Router
{
	...
	public struct RoutingPath : IEquatable<RoutingPath>
    {
        ...
        {
            if ((other._componentsValue?.Length ?? 0) == 0) return true;

            if ((_componentsValue?.Length ?? 0) == 0) return false;

            if (other._componentsValue.Length > _componentsValue.Length) return false;

            for (var i = 0; i < other._componentsValue.Length; i++)
            {
                var component = _componentsValue[i];
                var otherComponent = other._componentsValue[i];
                if (!string.Equals(component, otherComponent)) return false;
            }
        ...
        {
            if (obj is RoutingPath path) return Equals(path);
        ...
        {
            if (!TryRemoveBasePath(basePath, out var path))
        ...
        {
            if (other._componentsValue == null) return Clone();

            if (_componentsValue == null) return other.Clone();

            var componentCount = _componentsValue.Length + other._componentsValue.Length;
        ...
        {
            if (_componentsValue == null) return "/";
        ...
        {
            if (_componentsValue == null || _componentsValue.Length == 0) return new RoutingPath();
